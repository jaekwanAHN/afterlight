...
function item(s: GameState, kind: "magnet" | "bomb", x = 0, y = 0) {
  const i = {
    id: s.nextId++,
    kind,
    x,
...
it("spawns rarely, never before the first window, and caps alive items", () => {
  const s = createState();
  ...
  expect(delay).toBeGreaterThanOrEqual(ITEM_CONFIG.minInterval);
  expect(delay).toBeLessThanOrEqual(ITEM_CONFIG.maxInterval);
  expect(s.itemTimer).toBe(delay);
  spawnItems(s, delay, () => 0.5);
  spawnItems(s, delay, () => 0.5);
  expect(s.items).toHaveLength(ITEM_CONFIG.maxAlive);
...
it("items expire and are ignored when out of reach", () => {
  const s = createState();
  const i = item(s, "bomb", 500, 0);
  collectItems(s, 1);
  expect(s.items).toHaveLength(1);
  expect(i.life).toBe(ITEM_CONFIG.lifetime - 1);
  collectItems(s, ITEM_CONFIG.lifetime);
  expect(s.items).toHaveLength(0);
